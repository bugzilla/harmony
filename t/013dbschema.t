...
# A few exceptions are removed from the above list because Bugzilla (or its
# extensions) already use them as table/column names and always quote
# identifiers, so they are safe on every supported backend:
#   VALUE, TYPE, ALIAS, COALESCE  (core)
#   ACTION, DATA, DOMAIN, TIMESTAMP  (used by AntiSpam, BugmailFilter, Push
#     and RequestNagger tables)
...
foreach my $table (@tables) {
  ...
  if (scalar @reserved) {
    ok(0, "Table $table uses reserved words: " . join(", ", @reserved));
  }
  else {
